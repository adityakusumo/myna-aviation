...
<head>
    ...
    <title>{{ __('Forgot your password?') }} — Myna Aviation</title>
...
<div class="auth-card">
    <div class="auth-header">
        <h4><i class="bi bi-shield-lock me-2"></i>{{ __('Forgot your password?') }}</h4>
        <p>{{ __('Send Reset Link') }}</p>
    </div>
    <div class="p-4 pt-0">
        ...
        <form method="POST" action="{{ route('password.send') }}">
            ...
            <button type="submit" class="btn-myna btn-myna-primary py-2"><i class="bi bi-send me-1"></i>{{ __('Send Reset Link') }}</button>
        </form>
        <hr class="my-3" style="border-color:#eee;">
        <p class="text-center small mb-0"><a href="{{ route('auth.login.show') }}" class="text-decoration-none">{{ __('Back to Login') }}</a></p>
    </div>
</div>
</body>
</html>